...
> `prisma/init.db` 仅含 SQLite 业务数据，**不含** `data/uploads/` 下的上传文件（订单凭证、产品相册/规格缩略图等）。新环境首次部署后这些文件需重新上传，或从备份恢复 uploads 卷。
...
- [ ] v0.7+ 升级：无数据库变更；建议在手机浏览器抽查登录、订单列表卡片、账期核销付款弹窗
- [ ] v0.8+ 升级：含产品相册/分享、订单分享、客户 360、职能工作台等新表字段（entrypoint 自动 migrate）；**首次从 v0.7 升级请确认 `docker-compose.yml` 已挂载 `maofu-uploads` 卷**（见 8.4 节）
- [ ] 抽查：首页统计、订单、账期核销、客户跟进、收货信息、订单凭证、客户生日提醒
- [ ] v0.8+：**职能工作台**（ops01）、**产品管理/产品展示**、**客户 360**、**订单/产品分享链接**（分享页无需登录）
- [ ] v0.8+：在手机浏览器验证登录页、订单管理卡片列表、核销付款数量输入
...
| v0.8+ | 职能工作台、产品相册/零售价/销售展示、微信分享、客户 360、订单分享；产品参数（酒精度/香型/产地）、规格缩略图与描述 | 通常仅需 migrate；**务必挂载并备份 uploads 卷** |

详细字段说明见历史版本记录；当前仓库版本 **v0.8.0**。

### 7.3.1 v0.8.0 功能与路由说明

| 模块 | 说明 | 访问路径 / 备注 |
|------|------|-----------------|
| 职能工作台 | 待发货、待收款、部分收款、账期待办；分页每页 20 条 | `/workbench`（仅 **OPERATIONS** 角色；登录后自动跳转） |
| 产品管理 | 相册、酒精度/香型/产地、规格零售价体系、规格缩略图与描述 | `/products`（管理员） |
| 产品展示 | 销售侧产品目录，可生成客户分享链接 | `/catalog` |
| 产品分享页 | 公开页，仅显示零售指导价与零售红线价 | `/share/product/[token]` |
| 订单分享页 | 公开页，隐藏成本/毛利，手机号脱敏 | `/share/order/[token]` |
