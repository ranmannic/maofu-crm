...
### 7.3.6 post-v1.0.0：销售管理与产品软删除

自 **v1.0.0 标签** 之后、`main` 上的增量发布。从已运行 v1.0.0 的生产线升级时，在 [6.0 节](#60-生产更新安全流程推荐按顺序执行) 备份后拉代码并重建镜像；entrypoint 会自动 `prisma migrate deploy`。

#### 新增迁移（相对 v1.0.0）

| 迁移目录 | 变更内容 | 老数据影响 |
|----------|----------|------------|
| `20260724120000_product_spec_soft_delete` | `Product.deletedAt`、`ProductSpec.deletedAt` 及索引 | 已有产品/规格 **NULL**，行为与升级前一致；「删除」改为软删除，订单/提成/库存仍引用原记录 |
| `20260724150000_sales_manager_role` | `User.salesTeamName`、`canViewCustomerContact`（默认 `false`）、`salesManagerId` 及索引；角色枚举新增 **`SALES_MANAGER`**（SQLite 存 TEXT，无需改 enum 表） | 现有用户新列为 **NULL / false**，权限不变 |

> ✅ **数据安全要点**
>
> 1. **升级前必做**：备份 `prod.db` 与 uploads 卷；确认 **`INIT_DB=false`**。
> 2. **无需 SQL 回填**：迁移仅为加列与索引；软删除列默认未删除。
> 3. **无需** `db:init` / `db:seed` / `db:reset`。
> 4. **建议**：若首页或提成统计缺历史业绩，按需执行 `npx tsx prisma/sync-performance.ts`（或临时 `RUN_SYNC=true` 启动一次，见 [6.2 节](#62-临时开启启动时自动回填)）。

#### 升级后运维（必做）

1. **创建销售管理账号**（管理员）：「系统管理 / 账号管理」→ 新建角色 **销售管理**，或通过编辑已有用户改角色。
2. **小队配置**（同一页「小队配置」）：为每位销售管理设置 **小队名称**、勾选其管辖的 **销售** 账号；可选开启「可查看客户联系方式」（否则客户页不显示手机号/拨号）。
3. **HTTP 访问**：若生产仅用 HTTP，在 `.env` 设置 **`COOKIE_SECURE=false`** 并重启，避免登录后点菜单退回登录页（见 [5.2 节](#52-环境变量) 与 [Q11](#q11登录成功但点击菜单又退回登录页)）。

#### 功能摘要

| 模块 | 销售管理（SALES_MANAGER） |
|------|---------------------------|
| 数据概览 | 与管理员类似的统计块（**不含盈利/固定成本**）；下方 **各小队业绩**（全员小队，非仅本队） |
| 客户 / 跟进 | 管辖小队范围内 **只读**；不可新建/编辑客户或跟进 |
| 订单 / 账期 | 与小队销售相同操作面（创建订单等），数据 **按小队销售过滤**；不可删订单 |
| 产品 / 库存 | 与销售相同只读/展示能力（依版本与导航配置） |
...
docker compose up -d
docker compose exec app npx prisma migrate status
# 应看到 20260724120000_* 与 20260724150000_* 已 apply
docker compose exec app npx tsx prisma/sync-performance.ts   # 按需
curl -I http://127.0.0.1:3001/login
```
...
1. **HTTP 部署**：生产环境默认 `Secure` Cookie，纯 HTTP 访问时设置 `COOKIE_SECURE=false` 后重启应用。
2. **访问地址一致**：全程使用同一主机名（不要混用 `localhost` 与 `127.0.0.1` 或 IP）。
3. **多账号同一浏览器**：系统仅保存一个 `maofu_session` Cookie，后登录账号会覆盖先登录者（非服务端互踢；`UserSession` 表当前未参与鉴权）。
4. **多实例 JWT 不一致**：多台机器/容器须使用相同 `JWT_SECRET`。

### Q12：误执行了 db:init 或删除了 prod.db
...
### Q13：migrate deploy 失败（P3009 / failed migration）
